...
export function PlaceList({ places, onPlaceClick }: PlaceListProps) {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30 md:left-auto md:top-4 md:bottom-4 md:right-auto md:left-4 md:w-80">
      <div className="bg-white/95 backdrop-blur-md md:rounded-2xl rounded-t-2xl shadow-xl border border-[var(--sg-border)] overflow-hidden">
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="w-full flex items-center justify-between px-4 py-3 cursor-pointer
            hover:bg-[var(--sg-offwhite)] transition-colors"
        >
          <span className="text-sm font-semibold text-[var(--sg-navy)]">
            Places ({places.length})
          </span>
          {isExpanded ? (
            <ChevronDown size={18} className="text-[var(--sg-navy)]/60" />
          ) : (
            <ChevronUp size={18} className="text-[var(--sg-navy)]/60" />
          )}
        </button>
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              <div className="max-h-[50vh] md:max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar divide-y divide-[var(--sg-border)]">
                {places.map((place) => {
